...
class PonFDBTable extends CDataAbstractModule
{
    ...
    private function processWithInterface($interface) {
        ...
        foreach ($this->getOntIdsByInterface($interface) as $id) {
        ...
        foreach ($this->snmp->walk($oids) as $resp) {
            ...
            foreach ($wr->fetchAll() as $r) {
                $interface = $this->parseInterface(Helper::getIndexByOid($r->getOid(), 3));
                $interface['uni'] = Helper::getIndexByOid($r->getOid(), 1);
                $RETURN["{$interface['onu_id']}-{$r->getHexValue()}"] = [
                    '_id' => $interface['onu_id'],
                    '_interface' => $interface,
                    'interface' => $interface['name'] . ":" . $interface['onu_num'] . "/" . $interface['uni'],
        ...
        foreach ($response as $resp) {
            ...
            foreach ($wr->fetchAll() as $r) {
                ...
                $mac = Helper::oid2macArray([
                    ...
                    $RETURN["{$interface['onu_id']}-{$mac}"]['vlan_id'] = $vlanId;
            }
        }
        return array_values($RETURN);
    }*/
...
//         if($filter['interface']) {
//            $this->response = $this->processWithInterface($filter['interface']);
//         } else {
