...
class FdbTableTelnet extends CDataAbstractModule
{
    ...
    protected function executeByInterfaces($ifaces)
    {
        ...
        foreach ($ifaces as $iface) {
            ...
            if ($iface['type'] === 'ONU') {
                $resp = $this->telnet->exec("show mac-address ont {$m[2]}/{$m[3]}/{$m[4]} {$m[5]}\t\n");
                foreach (explode("\n", $resp) as $line) {
                ...
                $resp = $this->telnet->exec("show mac-address port {$telnetIdent}  with-ont-location\t\n");
                foreach (explode("\n", $resp) as $line) {
                    if (preg_match('/^([[:xdigit:]]{2}:[[:xdigit:]]{2}:[[:xdigit:]]{2}:[[:xdigit:]]{2}:[[:xdigit:]]{2}:[[:xdigit:]]{2})\s*([0-9]{1,4})\s*(\S*)\s*([0-9]{1,3})\s*(\S*)$/', trim($line), $match)) {
                        $responses[] = [
                            'mac_address' => $match[1],
                ...
                $resp = $this->telnet->exec("show mac-address port {$telnetIdent}");
                foreach (explode("\n", $resp) as $line) {
