...
class OntVendorInfo extends CDataAbstractModuleFD17xxV3
{
    /**
     * @var WrappedResponse[]
     */
    ...
    private function processNoInterface($response) {
        ...
        foreach ($this->getResponseByName('ont.verSoftware', $response)->fetchAll() as $r) {
            ...
            if($return[$onuId]['ver_software'] === '4E 75 6C 6C 00 ') $return[$onuId]['ver_software'] = null;
        }
    ...
    private function processWithInterface($interface, $response) {
        ...
        foreach ($responses as $r) {
            ...
            $issetIds[$onuId] = true;
            switch ($oid->getName()) {
                case 'ont.verSoftware': 
                    $return[$onuId]['ver_software'] = $wr->getValue();
                    if($return[$onuId]['ver_software'] === '4E 75 6C 6C 00 ') $return[$onuId]['ver_software'] = null;
                    break;
                case 'ont.verHardware': $return[$onuId]['ver_hardware'] = $wr->getValue(); break;
                case 'ont.vendor': $return[$onuId]['vendor'] = $wr->getParsedValue(); break;
