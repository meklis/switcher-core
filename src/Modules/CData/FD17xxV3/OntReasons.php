...
class OntReasons extends CDataAbstractModuleFD17xxV3
{
    /**
     * @var WrappedResponse[]
     */
    ...
    private function processNoInterface($response) {
        ...
        foreach ($this->getModule('pon_onts_status')->run()->getPrettyFiltered(['meta' => 'yes']) as $onts) {
            $return[$onts['interface']['_snmp_id']]['interface'] = $onts['interface'];
        }
    ...
    public function run($filter = [])
    {
        ...
        if(!$filter['interface']) {
            ...
            $oids = [];
            foreach ($this->getOntIdsByInterface($filter['interface']) as $snmp_id) {
                foreach ($optical as $oid) {
                    $oids[] = Oid::init("{$oid->getOid()}.{$snmp_id}");
                }
            }
