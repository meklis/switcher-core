...
abstract class ModuleAbstract extends AbstractModule
{
        ...
        foreach ($resp['if.Name']->fetchAll() as $f) {
            if (preg_match('/^(gpon|epon|gei|xgei)[-_]([0-9]{1,3})\/([0-9]{1,3})\/([0-9]{1,3})$/', trim($f->getValue()), $m)) {
                ...
                $response["{$m[2]}/{$m[3]}/{$m[4]}"] = [
                    ...
                    '_oid_id' => in_array($m[1], ['epon', 'gpon']) ? $this->encodeSnmpOid("{$m[1]}-olt_{$m[2]}/{$m[3]}/{$m[4]}") : null,
                    '_oid_eth_id' => in_array($m[1], ['epon', 'gpon']) ? $this->encodeSnmpOid("{$m[1]}-olt_{$m[2]}/{$m[3]}/{$m[4]}", "gpon_eth") : null,
                    '_xpon_id' => null,
                ];
    ...
    public function parseInterface($name, $parseBy = 'id')
    {
        ...
        $find = array_filter($xidList, fn($item) => $item['_xid'] == $name);
        if (count($find) > 0) {
        ...
        if (is_numeric($name)) {
            ...
            $find = array_filter($xidList, fn($item) => $item['id'] == $name);
            if (count($find) > 0) {
                return array_values($find)[0];
            }
