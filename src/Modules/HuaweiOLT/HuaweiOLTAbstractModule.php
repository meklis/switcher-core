...
abstract class HuaweiOLTAbstractModule extends AbstractModule
{
    ...
    protected function getInterfaces() {
        ...
        foreach ($this->getResponseByName('if.Name', $data)->fetchAll() as $iface) {
            ...
            if(preg_match('/^GPON ([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{1,2})$/', $iface->getValue(), $matches)) {
                $interfaces[$id] = [
                    'id' => $id,
                    'xid' => $xid,
                    'name' => $iface->getValue(),
            ...
            if(preg_match('/^ethernet([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{1,2})$/', $iface->getValue(), $matches)) {
                $interfaces[$id] = [
                    'id' => $id,
                    'xid' => $xid,
                    'name' => $iface->getValue(),
    ...
    private function findParentByName($name) {
        if(preg_match('/^GPON ([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{1,2})$/', $name, $matches)) {
            $id =  200000;
            $id += 10000 * $matches[1];
            $id += 100 * $matches[2];
            $id += $matches[3];
    ...
    private function getIdByName($name) {
        if(preg_match('/^GPON ([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{1,2}):([0-9]{1,3})$/', $name, $matches)) {
            $id =  200000000;
